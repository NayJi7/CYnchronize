...
CREATE MATERIALIZED VIEW MV_LOCATION
    ...
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.LOCATION@dblink_cergy;

CREATE MATERIALIZED VIEW MV_TYPE_MATERIEL
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.TYPE_MATERIEL@dblink_cergy;

CREATE MATERIALIZED VIEW MV_CONSTRUCTEUR
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.CONSTRUCTEUR@dblink_cergy;

CREATE MATERIALIZED VIEW MV_MODELE
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.MODELE@dblink_cergy;

CREATE MATERIALIZED VIEW MV_TYPE_EQUIPEMENT_RESEAU
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.TYPE_EQUIPEMENT_RESEAU@dblink_cergy;

CREATE MATERIALIZED VIEW MV_GROUPE
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.GROUPE@dblink_cergy;

CREATE MATERIALIZED VIEW MV_PROFIL
    BUILD IMMEDIATE
    REFRESH FAST ON DEMAND
    WITH PRIMARY KEY
    AS SELECT * FROM GLPI_OWNER.PROFIL@dblink_cergy;
